...
<template>
  <section class="text-white text-left w-[80dvw] md:w-1/2 pb-5">
    ...
    <p class="indent-5">
      Hover tooltips and explanators are absurdly anoying to setup and very external
    </p>
    <h2 class="text-xl">
      Details:
    </h2>
    <p class="indent-5">
      How can we use the new css Anchor properties to configure different types of tooltips
    </p>
    <h2 class="text-xl">
      Logic:
    </h2>
    ...
    <p class="indent-5">
      Given that the anchored element is always in the dom we will need remove these as they tooltip, even at opaicty-0, will sit above other elements in the base css stack, this can be cleanly solved with <RouterLink to="/discrete-animations">
        discrete animations
      </RouterLink>
    </p>
  </section>
  <section class="flex gap-x-4">
    ...
    <button
      class="[anchor-name:_--anchor-toggle-el] border-red-500 border border-solid px-4 py-2"
      @click="toggleHelper"
    >
      Toggle
    </button>
    ...
    <button
      class="border-red-500 border border-solid px-4 py-2"
      @click="() => toggleDialog(true)"
    >
      What about in modals?
    </button>
    <dialog 
      ref="modalRef"
      class="
    ...
    >
      <div class="flex items-center justify-center h-60 aspect-square">
        <button class="[anchor-name:_--anchor-hover-inModal-el] border-blue-500 border border-solid px-4 py-2 peer/hover">
          Hover
        </button>
        <div 
          :class="[
