  ...
  <section class="flex flex-col w-fit gap-y-8 justify-start h-[150%] mt-20">
    <article>
      ...
      <select
        v-if="options.size > 0"
        id="nativeSelect_id"
        class="text-slate-700 bg-slate-900 border border-emerald-600 rounded-sm"
      >
        <template
          v-for="(key, index) in [...options.keys()]"
          :key="`${key}_${index}`"
        >
          <option
            :value="key"
    ...
    <article>
      ...
      <p class="ml-5">
        Note that both a visual <Highlight content="span" /> and the input field on the input box are both set
      </p>
    ...
    <article>
      ...
      <p class="ml-5">
        Note that this looks horrible and can leave the user confused
      </p>
      <section class="absolute">
        <div 
          aria-hidden
        ...
        >
          I'm a fake placeholder adasdasd
        </div>
        <multiselect
